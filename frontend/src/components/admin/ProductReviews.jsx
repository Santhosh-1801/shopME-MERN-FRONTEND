...
const ProductReviews = () => {
    ...
    const setReviews=()=>{
        ...
        data?.reviews?.forEach((review)=>{
          reviews.rows.push({
            ...
            actions:(<>
            <button className='btn btn-outline-danger ms-2' onClick={()=>deleteReviewHandler(review?._id)} disabled={isDeleteLoading}>
 ...
 return (
    ...
    {data?.reviews?.length>0 ?<MDBDataTable data={setReviews()} className='px-3' bordered striped hover/>:<p className='mt-5 text-center'>No Reviews Found</p>}
    </AdminLayout>
  )
}
